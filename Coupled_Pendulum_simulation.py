...
class Coupled_Pendulum:
    ...
    def show_particle_and_trajectory(self):
        ...
        def animate(i): 
            ...
            tj1x.append(self.x1[i])
            tj1y.append(self.y1[i])
            tj2x.append(self.x2[i])
            tj2y.append(self.y2[i])
            t1,t2=sline(0,0,self.x1[i],self.y1[i])
            s1,s2=sline(self.l0,0,self.x2[i],self.y2[i])
            #sp1,sp2=sline(self.s1x[i],self.s1y[i],self.s2x[i],self.s2y[i])
            ...
            line3.set_data(tj1x,tj1y)
            line4.set_data(tj2x,tj2y)
